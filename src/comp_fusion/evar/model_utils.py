...
# [Hyosun] added: [검증필요] 이렇게 순차적으로 embed_dim줄이고 sequence_len 줄여도 되나
def mean_pooling(type_embeds = 'patch', patch_embeddings=True, dim=-1):
    assert len(patch_embeddings.shape) == 3 # Batch,sequence_len,embed_dim
    x2 = torch.mean(patch_embeddings, dim=dim) #[Hyosun] first reduce each embeds_dim
    x2 = torch.mean(patch_embeddings, dim=dim) #[Hyosun] second reduce sequence_len
    return x2
#[/Hyosun] added
