...
# [Hyosun] current path: ./src/finetune/CornellBirdcall/ [/Hyosun]
...
labels_arr = np.loadtxt('./data/cbc_labels.csv', delimiter='\t', dtype='str')#, skiprows=1)
print("labels_arr: \n", labels_arr)
...
if os.path.exists('./data/birdsong_class_labels_indices.csv') == False:
    index = []
    mid = []
    ...
    df.to_csv('./data/birdsong_class_labels_indices.csv', index=False)  
...
os.path.exists('./data/evaluation_setup/fold1_test.csv') == False:
    ...
    base_dir = './data/BirdSong_audio_16k_10s/'
    import random
    train_dict_list = []
    test_dict_list = []
    labels = []
...
if os.path.exists('./data/datafiles') == False:
    ...
    base_path = os.path.abspath(os.getcwd()) + "/data/BirdSong_audio_16k_10s/"

    if os.path.exists('./data/birdsong_class_labels_indices.csv') == True and \
    os.path.exists('./data/evaluation_setup') == True :
        ...
        with open('./data/datafiles/birdsong_train_data_1' +'.json', 'w') as f:
            json.dump({'data': train_wav_list}, f, indent=1)

        with open('./data/datafiles/birdsong_test_data_1' +'.json', 'w') as f:
...
print('[Hyosun] Finished BirdSong Data Preparation')