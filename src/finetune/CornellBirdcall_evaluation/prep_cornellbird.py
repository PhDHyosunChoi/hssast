...
birdsong_path = './data/BirdSong/' #[Hyosun] BirdSong wav audio files converted from BirdSong_mp3 using prep_birdsong_convert_mp3_wav.py
...
if os.path.exists('./data/cornellbird_class_labels_indices.csv') == False:
    index = []
    mid = []
    ...
    df.to_csv('./data/cornellbird_class_labels_indices.csv', index=False)  

    print("[Hyosun] create labels_info.csv file: All Finished!!")
# [/Hyosun] create labels_info.csv file including only labels information =========================

# [Hyosun: create the label description dictionary] 
if os.path.exists('./data/cornellbird_class_labels_indices.csv') == True:    
    label_set = np.loadtxt('./data/cornellbird_class_labels_indices.csv', delimiter=',', dtype='str')
    label_map = {}
    ...
    print(label_map)
    print("[Hyosun] create the label description dictionary: using cornellbird_class_labels_indices.csv finished!!")
#[/Hyosun: create the label description dictionary] 
...
if os.path.exists('./data/datafiles') == False:
    ...
    base_path = os.path.abspath(os.getcwd()) + "/data/BirdSong_audio_16k/"

    if os.path.exists('./data/cornellbird_class_labels_indices.csv') == True and \
    os.path.exists('./data/evaluation_setup') == True :
        ...
        with open('./data/datafiles/cornellbird_train_data_1' +'.json', 'w') as f:
            json.dump({'data': train_wav_list}, f, indent=1)

        with open('./data/datafiles/cornellbird_test_data_1' +'.json', 'w') as f:
...
print('[Hyosun] Finished CornellBird Data Preparation')