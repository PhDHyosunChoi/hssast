...
else: #os.path.exists('./data/event_04s') == True:
    if os.path.exists('./data/event_04s_audio_16k/') == False:
        # convert all samples to 16kHZ : #[Hyosun] repositioned 
        print('Now converting all event_04s audio to 16kHz')
        #[/Hyosun] repositioned 

        # convert the audio to 16kHz
        base_dir = './data/event_04s/'
        out_dir  = './data/event_04s_audio_16k/'
        os.mkdir('./data/event_04s_audio_16k/')
        audio_list = get_immediate_files('./data/event_04s')
        for i in range(0,len(labels_arr)):
            os.mkdir('./data/event_04s_audio_16k/'+labels_arr[i]) #[Hyosun] no directory error fix
            random.seed(1981)
...
if os.path.exists('./data/event_04s_class_labels_indices.csv') == False:
    index = []
    mid = []
    ...
    df.to_csv('./data/event_04s_class_labels_indices.csv', index=False)  
...
os.path.exists('./data/evaluation_setup/fold1_test.csv') == False:
    ...
    base_dir = './data/event_04s_audio_16k/'
    import random
    train_dict_list = []
    test_dict_list = []
    labels = []
...
if os.path.exists('./data/datafiles') == False:
    ...
    base_path = os.path.abspath(os.getcwd()) + "/data/event_04s_audio_16k/"

    if os.path.exists('./data/event_04s_class_labels_indices.csv') == True and \
    os.path.exists('./data/evaluation_setup') == True :
        ...
        with open('./data/datafiles/event_04s_train_data_1' +'.json', 'w') as f:
            json.dump({'data': train_wav_list}, f, indent=1)

        with open('./data/datafiles/event_04s_test_data_1' +'.json', 'w') as f:
...
print('[Hyosun] Finished event_04s Data Preparation')