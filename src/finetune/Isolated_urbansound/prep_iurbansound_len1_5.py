...
if os.path.exists('./data/iurban') == False:
    ...
    if os.path.exists('./data/iurban_1_5s/') == False:
        ...
        out_dir  = './data/iurban_1_5s/'
        os.mkdir('./data/iurban_1_5s/')

        for i in range(0,len(labels_arr)):
            os.mkdir('./data/iurban_1_5s/'+labels_arr[i]) #[Hyosun] no directory error fix
            random.seed(1981)
            ...
            for audio in audio_list_each_label: 
                ...
                s_sec = 0 
                e_sec = 1.5
                while 3/2<= audio_len:   #[Hyosun] drop the last clip less than 2s                                 
