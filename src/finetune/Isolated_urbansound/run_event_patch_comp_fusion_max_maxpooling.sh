...
# prep iurbanevent dataset and download the pretrained model
if [ -e data/datafiles ]
then
    echo "iurbanevent already downloaded and processed."
else
    python prep_iurbanevent.py
fi
...
#[Hyosun] iurbanevent setting: edited for iurbanevent_04s mean=-0.2408463 std=1.5087016
## -- target_length 바꾸면 acc 올라가나?? length 도 바꿀까, 1024정도로?? 2023-10-04
dataset=iurbanevent
dataset_mean=-0.2408463
dataset_std=1.5087016
target_length=512
noise=True
#[/Hyosun] iurbanevent setting: edited for iurbanevent_04s mean=-0.2408463 std=1.5087016
...
lr=1e-4 #1e-5 #1e-4 [Hyosun] original lr
freqm=24
timem=96
mixup=0
...
mlp_layers=2 #4 #6 #4 #2 #6 #4 #2 #6 #4 #2 #6 #4
#[/Hyosun] comp_fusion logic added
...
do
  ...
  tr_data=./data/datafiles/event_04s_train_data_${fold}.json
  te_data=./data/datafiles/event_04s_test_data_${fold}.json

  CUDA_CACHE_DISABLE=1 python -W ignore ../../run.py --dataset ${dataset} \
  --data-train ${tr_data} --data-val ${te_data} --exp-dir $exp_dir \
  --label-csv ./data/event_04s_class_labels_indices.csv --n_class 11 \
...
#[Hyosun] editied: --n_class 10 -> 11          [/Hyosun]
#[Hyosun] parser.add_argument('--lr', '--learning-rate', default=0.001, type=float, metavar='LR', help='initial learning rate') 

#[Hyosun] edited 2023-10-17
#python ./get_esc_result.py --exp_path ${base_exp_dir}
python get_event_result.py --exp_path ${base_exp_dir} #요거이용하기(2023-10-02)
#[/Hyosun] edited 2023-10-17