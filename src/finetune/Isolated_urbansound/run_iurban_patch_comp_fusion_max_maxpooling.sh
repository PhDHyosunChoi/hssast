...
# prep iurbansound dataset and download the pretrained model
if [ -e data/datafiles ]
then
    echo "iurbansound already downloaded and processed."
else
    python prep_iurbansound.py
fi
...
#[Hyosun] iurbansound setting: edited for iurbanosund mean=-0.48460108 std=1.8269863
## -- target_length 바꾸면 acc 올라가나?? length 도 바꿀까, 1024정도로?? 2023-10-04
dataset=iurbansound
dataset_mean=-0.48460108
dataset_std=1.8269863
target_length=512
noise=True
#[/Hyosun] iurbansound setting: edited for iurbanosund mean=-0.48460108 std=1.8269863
...
mlp_layers=6 #4 #2 #6 #4 #2 #6 #4 #2 #6 #4
#[/Hyosun] comp_fusion logic added
...
do
  ...
  tr_data=./data/datafiles/iurban_1s_train_data_${fold}.json
  te_data=./data/datafiles/iurban_1s_test_data_${fold}.json

  CUDA_CACHE_DISABLE=1 python -W ignore ../../run.py --dataset ${dataset} \
  --data-train ${tr_data} --data-val ${te_data} --exp-dir $exp_dir \
  --label-csv ./data/iurban_1s_class_labels_indices.csv --n_class 50 \
...
python get_iurban_result.py --exp_path ${base_exp_dir} #요거이용하기(2023-10-02)
#[/Hyosun] edited 2023-10-17