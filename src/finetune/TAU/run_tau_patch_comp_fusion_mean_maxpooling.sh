...
if [ -e data/datafiles ]
then
    echo "tau already downloaded and processed."
else
    python prep_tau.py
fi
...
dataset=tau
dataset_mean=-4.241041
dataset_std=3.0622184
target_length=512
noise=True
#[/Hyosun] tau setting: edited for tau mean=-4.241041 std=3.0622184
...
timem=96
mixup=0
epoch=2 #35 #50 #10 #50 #50 #5  #50 [Hyosun] The original epoch 50: reduced into 5 for experiments
batch_size=48
fshape=16
tshape=16
fstride=10
...
base_exp_dir=./exp/"$(date +'%Y-%m-%d/%H:%M:%S%p')"-test01-${dataset}-comp_fusion-${comp_fusion}-comp_fusion_method-${comp_fusion_method}-comp_fusion_multi_layer-${comp_fusion_multi_layer}-pooling-${pooling_ty}-mlp${mlp_layers}-loss-${loss}-f${fstride}-${fshape}-t${tstride}-${tshape}-b${batch_size}-lr${lr}-${task}-${model_size}-${pretrain_exp}-${pretrain_model}-${head_lr}x-noise${noise} 
...
do
  ...
  tr_data=./data/datafiles/tau_train_data_${fold}.json
  te_data=./data/datafiles/tau_eval_data_${fold}.json

  CUDA_CACHE_DISABLE=1 python -W ignore ../../run.py --dataset ${dataset} \
  --data-train ${tr_data} --data-val ${te_data} --exp-dir $exp_dir \
  --label-csv ./data/tau_class_labels_indices.csv --n_class 10 \
