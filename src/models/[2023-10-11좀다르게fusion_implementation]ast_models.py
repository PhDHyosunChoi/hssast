...
class ASTModel(nn.Module):
    ...
    def finetuningavgtok(self, x):#[Hyosun:fine-tuning] fusion here seems to be right.(Yes!!)
        ...
        #[Hyosun] 2023-10-11
        multi_feat_embeds_mean= [] #[Hyosun] added for comp_fusion
        multi_feat_embeds_max= [] #[Hyosun] added for comp_fusion
        multi_feat_embeds_min= [] #[Hyosun] added for comp_fusion
        #[/Hyosun] 2023-10-11
        ...
        #[Hyosun] added
            ...
            if self.comp_fusion == 'True':
                ...
                if blk_id in self.comp_fusion_multi_layer: #self.composing_multi_layer: #[Hyosun] 2023-10-11 editied [/Hyosun] 
                    x = self.v.norm(x) #[Hyosun] edited from xdash = self.v.norm(x) [/Hyosun]
                    print("[Hyosun:finetuningavgtok before hstack] x.shape: ", x.shape)
                    ...
                    if self.comp_fusion_method == 'use_all_patch': #[Hyosun:comment] only interested in this way for now
                        ...
                        #[Hyosun] 2023-10-11 Wed 요기에 합치기 =========================================================
                        #[Hyosun] mean + max pooling
                        if self.pooling_ty == 'mean_max': #[Hyosun] 2023-10-11 editied [/Hyosun]  

                            print("[Hyosun:finetuningavgtok:mean_max] x.shape: \n", x.shape)

                            #[Hyosun] comment out 2023-10-11
                            ...
                            #[Hyosun] meanpooling added #[Hyosun] 2023-10-11 editied
                            # average output of all tokens except cls token(s) [Hyosun] 여기 hstack후의 옆으로 붙은것도(M,L layers) cls떼야                            
                            xdash_mean = torch.mean(x[:, self.cls_token_num:, :], dim=1)
                            multi_feat_embeds_mean.append(xdash_mean)
                            print("[Hyosun:finetuningavgtok:mean_max] xdash_mean.shape: ",xdash_mean.shape)
                            #[/Hyosun] meanpooling added #[/Hyosun] 2023-10-11 editied

                            #[Hyosun] maxpooling added #[Hyosun] 2023-10-11 editied
                            (xdash_max, _) = torch.max(x[:, self.cls_token_num:, :], dim=1)
                            multi_feat_embeds_max.append(xdash_max)
                            print("[Hyosun:finetuningavgtok:mean_max] xdash_max.shape: ",xdash_max.shape)
                            #[/Hyosun] maxpooling added #[/Hyosun] 2023-10-11 editied

                        #[Hyosun] mean pooling  ==> try!![2023-10-11 오전]  
                        elif self.pooling_ty == 'mean':
                            print("[Hyosun:finetuningavgtok:mean] x.shape: \n", x.shape)
                        ...
                        #[Hyosun] max pooling
                            ...
                            print("[Hyosun:finetuningavgtok:max] x.shape: \n", x.shape)

                            #[Hyosun] maxpooling added #[Hyosun] 2023-10-11 editied
                            (xdash_max, _) = torch.max(x[:, self.cls_token_num:, :], dim=1)
                            multi_feat_embeds_max.append(xdash_max)
                            print("[Hyosun:finetuningavgtok:max] xdash_max.shape: ",xdash_max.shape)
                            #[/Hyosun] maxpooling added #[/Hyosun] 2023-10-11 editied

                        elif self.pooling_ty == 'mean_mean':
                            print("[Hyosun:finetuningavgtok:mean_mean] x.shape: \n", x.shape)

                            #[Hyosun] meanpooling added #[Hyosun] 2023-10-11 editied
                            # average output of all tokens except cls token(s) [Hyosun] 여기 hstack후의 옆으로 붙은것도(M,L layers) cls떼야                            
                            xdash_mean = torch.mean(x[:, self.cls_token_num:, :], dim=1)
                            multi_feat_embeds_mean.append(xdash_mean)
                            print("[Hyosun:finetuningavgtok:mean_mean] xdash_mean.shape: ",xdash_mean.shape)
                            #[/Hyosun] meanpooling added #[/Hyosun] 2023-10-11 editied

                        #[Hyosun] max + max pooling
                        elif self.pooling_ty == 'max_max':
                            print("[Hyosun:finetuningavgtok:max_max] x.shape: \n", x.shape)

                            #[Hyosun] maxpooling added #[Hyosun] 2023-10-11 editied
                            (xdash_max, _) = torch.max(x[:, self.cls_token_num:, :], dim=1)
                            multi_feat_embeds_max.append(xdash_max)
                            print("[Hyosun:finetuningavgtok:max_max] xdash_max.shape: ",xdash_max.shape)
                            #[/Hyosun] maxpooling added #[/Hyosun] 2023-10-11 editied
        ...
        #[Hyosun] 2023-10-11 Wed edited          
        if self.comp_fusion == 'True': 
            #[Hyosun] mean + max pooling
            if self.pooling_ty == 'mean_max':
                xdash_mean_fusion = torch.hstack(multi_feat_embeds_mean)
                xdash_max_fusion = torch.hstack(multi_feat_embeds_max)
                #xdash_mean_fusion = self.v.norm(xdash_mean_fusion)
                #xdash_max_fusion = self.v.norm(xdash_max_fusion)
                #[Hyosun] mean + max pooling 
            ...
            #[Hyosun] max pooling
                ...
                #print("[Hyosun:finetuningavgtok after mlp_head] xdash_mean.shape: ",xdash_mean.shape)
                return xdash_max_fusion 
            #[Hyosun] mean + mean pooling
            if self.pooling_ty == 'mean_mean':
                xdash_mean_fusion = torch.hstack(multi_feat_embeds_mean)

                #[Hyosun] mean + mean pooling 
                xdash_mean_mean_fusion = xdash_mean_fusion + xdash_mean_fusion
                #[/Hyosun]mean + mean pooling 

                #[Hyosun] go through mlp_head
                xdash_mean_mean_fusion = self.mlp_head(xdash_mean_mean_fusion)
                print("[Hyosun:finetuningavgtok:mean_mean] xdash_mean_mean_fusion.shape after go through self.mlp_head(): ",xdash_mean_mean_fusion.shape)
                #print("[Hyosun:finetuningavgtok after mlp_head] xdash_mean.shape: ",xdash_mean.shape)
                return xdash_mean_mean_fusion          
            #[Hyosun] max + max pooling
            elif self.pooling_ty == 'max_max':
                xdash_max_fusion = torch.hstack(multi_feat_embeds_max)
                #[Hyosun] max + max pooling 
                ...
                #print("[Hyosun:finetuningavgtok after mlp_head] xdash_mean.shape: ",xdash_mean.shape)
                return xdash_max_max_fusion 
        else:#[Hyosun] if self.comp_fusion == 'False' [/Hyosun]
