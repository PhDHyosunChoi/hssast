...
# [Hyosun] 이부분 pandas로 다시 만들어야함 그래야 바로 아래꺼 돌아가 !!
...
if os.path.exists('./data/tau_class_labels_indices.csv') == True:
    ...
    # # [Hyosun] test_meta: create test_data json file ==> No labels in the file ==> Therefore, we're not going to use it
    ...
    with open('./data/datafiles/tau_train_data_' +'.json', 'w') as f:
        json.dump({'data': train_wav_list}, f, indent=1)

    # with open('./data/datafiles/tau_test_data_' +'.json', 'w') as f:
    #     json.dump({'data': test_wav_list}, f, indent=1)

    with open('./data/datafiles/tau_eval_data_' +'.json', 'w') as f:
